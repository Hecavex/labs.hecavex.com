...
const apt31Record = payload.records.find(record => record.actor.id === 'apt31' && record.technique_id === 'T1598.003');
assert(apt31Record.assessment.alternatives.length > 0);
const assuranceBrief = api.toMarkdown(api.createEnvelope(data, [apt31Record.id], { actor: 'apt31' }, context, exportedAt));
for (const value of [apt31Record.assessment.mapping_rationale, apt31Record.assessment.source_dependence, apt31Record.temporal_scope.note, apt31Record.sources[0].source_identity.access_outcome]) assert(assuranceBrief.includes(api.markdownText(value)));
assert(assuranceBrief.includes('Source body SHA-256: Not recorded'));
