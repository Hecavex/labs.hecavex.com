    ...
    with sync_playwright() as playwright:
        ...
        for width in WIDTHS:
            ...
            for route in ROUTES:
                ...
                if route == "/baltic-threat-atlas/":
                    atlas_records = page.locator(".atlas-record")
                    atlas_records.first.wait_for(state="visible")
                    displayed_dates = atlas_records.locator(".atlas-date strong").all_text_contents()
                    assert displayed_dates == [
                        "2026-07-23", "2026-06", "2026-Q1", "2026-Q1",
                        "2025", "2025", "2025", "2024-09-05", "2024-Q2",
                        "2024-Q2", "2024", "2023-09", "2023-07-10",
                    ], f"Atlas observations are not newest-first at {width}px: {displayed_dates}"

                if route in {"/methodology/", "/licence/", "/security/"}:
                    prose_alignment = page.locator(".document-prose > p").evaluate_all(
                        "elements => elements.map(element => getComputedStyle(element).textAlign)"
                    )
                    assert prose_alignment and set(prose_alignment) == {"justify"}, f"document prose is not justified: {route} at {width}px: {prose_alignment}"

                if route == "/attack-map/guide/":
                    guide_alignment = page.locator(".guide-section-head p").evaluate_all(
                        "elements => elements.map(element => getComputedStyle(element).textAlign)"
                    )
                    assert guide_alignment and set(guide_alignment) == {"justify"}, f"guide prose is not justified at {width}px: {guide_alignment}"
