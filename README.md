...
The site is purpose-built static HTML, CSS and browser JavaScript. It shares the HECAVEX operational portfolio shell with the other HECAVEX properties, uses self-hosted Space Grotesk headings, Inter reading text and IBM Plex Mono technical data, and does not depend on a remote font or icon service. The [source map](docs/CODEMAP.md) identifies the responsible module and regression checks for each interface, dataset and release boundary.
